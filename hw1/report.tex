...
SPMV-OPENMP extends the CSR methodology to parallel execution with several locality-conscious techniques. The CSR arrays are first-touch initialized in parallel (\texttt{schedule(static), proc\_bind(spread)}) so each thread's rows land on the local NUMA node. Each row is sorted by column index to improve sequential access into \texttt{x}, and for medium-size vectors a per-thread replica of \texttt{x} eliminates cross-socket reads. The kernel distributes rows with the same \texttt{schedule(static)} that matches the first-touch layout, and the inner loop uses \texttt{omp simd reduction} to exploit vector units. Thread placement defaults to \texttt{OMP\_PROC\_BIND=spread} and \texttt{OMP\_PLACES=cores}. We also explored a more aggressive approach using SELL-C-16 with AVX-512 intrinsics that achieves significantly better performance but requires modifications beyond the student-implementable functions; see the Appendix for details.
...
\section*{Appendix: Experimental SELL-C-16 with AVX-512}
\label{sec:sell}
